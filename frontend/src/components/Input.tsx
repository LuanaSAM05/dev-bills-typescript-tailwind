...
const Input = ({ fullWidth, icon, label, error, id, className, ...rest }: InputProps) => {
    ...
    return (
        <div className={`${fullWidth ? 'w-full' : ''} mb-4`}>
            ...
            bg-gray-800 px-4 py-3 text-sm text-gray-50 transition-all focus:outline-none focus:ring-2 ${error ? "focus:border-red-500 focus:ring-red-500/2" : "focus:border-primary-500 focus:ring-primary-500/2"}
            ${icon ? "pl-10" : ""}
            ${className}
            `}
                {...rest}
